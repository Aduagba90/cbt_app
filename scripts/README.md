...
| Script | Purpose |
| --- | --- |
| `build_content_batch1.py` … `build_content_batch34.py` | Write `content/batch_00N_*.json` (JAMB questions, passages, retirement rules). Numerical answers are re-computed before the file is written; the script exits on any mismatch. Re-running is safe — output is deterministic. |
